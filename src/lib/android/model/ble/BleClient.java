...
public abstract class BleClient extends BluetoothGattCallback implements BluetoothAdapter.LeScanCallback {
    ...
    public boolean startScanning() {
        if (adapter.startLeScan(this)) {
            LogUtils.yanry("start BLE scanning...");
    ...
    public void stopScanning() {
        ...
        LogUtils.yanry("stop BLE scanning.");
    ...
    public boolean connect(BluetoothDevice device) {
        if (adapter != null) {
            outputStreams.clear();
            inputStreams.clear();
            LogUtils.yanry("Trying to create a new connection.");
    ...
    public void sendData(UUID serviceId, UUID characteristicId, String data) {
        if (gatt == null) {
            LogUtils.yanry("gatt is null.");
            onConnectionError();
        } else {
            BluetoothGattService service = gatt.getService(serviceId);
            if (service == null) {
                LogUtils.yanry("get service fail: %s", serviceId);
                onConnectionError();
            } else {
                BluetoothGattCharacteristic characteristic = service.getCharacteristic(characteristicId);
                if (characteristic == null) {
                    LogUtils.yanry("get characteristic fail: %s", characteristicId);
                    onConnectionError();
                } else if (gatt.setCharacteristicNotification(characteristic, true)) {
                    if (data == null) {
                        LogUtils.yanry("trying to read characteristic: %s.", characteristic.getUuid());
                        CommonUtils.retryOnFail(RETRY_TIMES_ON_FAIL, RETRY_INTERVAL, () -> handleCharacteristic(gatt, characteristic, true), () -> {
                            LogUtils.yanry("read characteristic fail.");
                            onConnectionError();
                        });
                    } else {
                        LogUtils.yanry("sending data (%s): %s", characteristicId.toString().substring(0, 3), data);
                        try {
                    ...
                    LogUtils.yanry("set characteristic notification fail.");
                    onConnectionError();
                }
            }
        }
    ...
    private void writeCharacteristic(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        CommonUtils.retryOnFail(RETRY_TIMES_ON_FAIL, RETRY_INTERVAL, () -> handleCharacteristic(gatt, characteristic, false), () -> {
            LogUtils.yanry("write characteristic fail.");
            onConnectionError();
        });
    }
    ...
    public void disconnect() {
        if (gatt != null) {
            LogUtils.yanry("disconnect gatt.");
            gatt.disconnect();
        }
    }
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        if (status == BluetoothGatt.GATT_SUCCESS) {
            ...
            LogUtils.yanry("discover %s services.", services.size());
            BleClient.this.onDiscoverServices(services);
        } else {
            LogUtils.yanry("discover services fail: %s.", status);
            onConnectionError();
        }
    }
    ...
    public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (pendingAction.isPending()) {
            ...
            if (status != BluetoothGatt.GATT_SUCCESS) {
                LogUtils.yanry("read characteristic(%s) fail: %s.", characteristic.getUuid(), status);
                onConnectionError();
            }
        } else {
            LogUtils.yanry("receive read characteristic result beyond timeout: %s!", status);
        }
    }
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (pendingAction.isPending()) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                LogUtils.yanry("write characteristic(%s) fail: %s.", characteristic.getUuid(), status);
                onConnectionError();
            }
        } else {
            LogUtils.yanry("receive write characteristic result beyond timeout: %s!", status);
        }
    }
    ...
    public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        if (gatt == BleClient.this.gatt) {
            ...
            UUID uuid = characteristic.getUuid();
            LogUtils.yanry("receive %s: %s", uuid.toString().substring(0, 3), Arrays.toString(value));
            if (Arrays.equals(value, Config.START_SIGNAL)) {
                ...
                if (value[0] == 0) {
                    try {
                        ...
                        LogUtils.yanry("receive data: " + data);
